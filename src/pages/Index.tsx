...
import { FileText, Gavel, Users, TrendingUp, RefreshCw, AlertTriangle, Shield } from "lucide-react";
  ...
  return <div className="min-h-screen bg-background">
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Análise em tempo real das ocorrências registradas pela GCM</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <KPICard title="Total de Registros" value={kpis.totalRegistros} icon={FileText} trend={{
          value: kpis.comparativoAno,
          label: "vs ano anterior"
        }} variant="info" />
          <KPICard title="Total de Prisões" value={kpis.totalPrisoes} icon={Gavel} variant="warning" />
          <KPICard title="Total de Presos" value={kpis.totalPresos} icon={Users} variant="destructive" />
          <KPICard title="Taxa de Prisão" value={`${kpis.totalRegistros > 0 ? (kpis.totalPrisoes / kpis.totalRegistros * 100).toFixed(1) : 0}%`} icon={TrendingUp} variant="success" />
        </div>

        {/* Análise Dinâmica */}
        <DynamicAnalysis filteredData={filteredData} allData={data} turnos={turnos} hourlyData={hourlyData} fatosDPRanking={fatosDPRanking} bairrosData={bairrosData} totalPrisoes={kpis.totalPrisoes} totalPresos={kpis.totalPresos} hasActiveFilters={hasActiveFilters} />

        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          {/* Filtros */}
          <div className="lg:col-span-1">
            <DashboardFilters filters={filters} onFiltersChange={setFilters} availableYears={availableOptions.years} availableBairros={availableOptions.bairros} availableFatos={availableOptions.fatos} />
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          <RankingTable title="Top 10 - Fatos DP" data={fatosDPRanking} />
          <RankingTable title="Top 10 - Ocorrências GCM" data={ocorrenciaGCMRanking} />
          <RankingTable title="Top 10 - Ruas" data={ruasRanking} />
        </div>

        {/* Bairros */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <RankingTable title="Distribuição por Bairros" data={bairrosData} maxItems={15} />
          <Card>
            <CardHeader>
